  ...
  document.body.querySelector(".loginButton").addEventListener("click", function() {
      if (usernameValid(inputUser) && passwordValid(inputPass)) {
        ...
        if (!usernameValid(inputUser) && passwordValid(inputPass)) {
          ...
          window.alert(
            "Either the username or password was incorrect, please try again"
          );
        }
      }
    });
...
function addGrade() {
  ...
  nameInputText.placeholder = "Student's name goes here";
  ...
  gradeInputInstructions.innerHTML =
    "Input grades below. First, insert student's FIRST & LAST name. Second, input grade from 0-100. Then, click 'submit.'";
    ...
    document.body.querySelector(".submitButton").addEventListener("click", function() {
      if (inputValid(nameInput) && gradeValid(gradeInput)) {
        ...
        if (!inputValid(nameInput) && gradeValid(gradeInput)) {
          ...
          window.alert("You didn't correctly put in a grade between 0 & 100. \n\n⚠️ Do NOT include a percentage % ⚠️.");
          
        } else {
          window.alert(
            "Come on man, you didn't even input: \n- Student Name \n- Student Grade\n    OR \n- You inputted info wrong. \n\nStop being lazy. USE THIS GRADES APPLICATION OR ELSE!");
        }
      }
    });
